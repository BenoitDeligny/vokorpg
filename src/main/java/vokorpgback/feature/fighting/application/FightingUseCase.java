...
public class FightingUseCase {
    ...
    public CombatResult handle(Fight fight) {
        ...
        if (fight.attemptToFlee() && (computeCharacterFleeingScore(characterFighter) > computeFightingMonsterTotalPower(monsters))) {
    ...
    private List<MonsterFighter> computeRemainingMonsters(
        ...
        return IntStream.range(0, monsters.size())
                .mapToObj(i -> applyDamagesToMonsters(monsters.get(i), characterDamage, i, characterFighter.getAgility()))
                .filter(monster -> isAlive(monster, monsters))
                .collect(Collectors.toList());
    }

    private MonsterFighter applyDamagesToMonsters(MonsterFighter monster, int characterDamage, int index, int numberOfMonstersFaced) {
    ...
    private CharacterFighter computeRemainingCharacterPower(List<MonsterFighter> monsters, CharacterFighter character) {

        int totalDamage = monsters.stream().limit(character.getAgility()).mapToInt(MonsterFighter::rollDamage).sum();
